...
function tb_bbs_goUpdate(hb_no) {
	...
	$.getJSON(url, function (data) {
		var hb_content = data.hb_content;
			hb_content = hb_content.replace(/<br>/gi, "\n");
		$("#hb_content").val(hb_content);
		$("#hb_contentUpdate2").val(data.hb_no);
	});
}
function tb_bbs_write() {
	$(document).on("click","#tb_bbs_button1",function() {
		var hb_content = $("#hb_content").val();
			hb_content = hb_content.replace(/\n/gi, "<br>");
		location.href="teabag.bbs.write?hb_content=" + hb_content;
	});
...
function tb_bbs_update() {
	$(document).on("click", "#tb_bbs_button2", function() {
		var hb_content = $("#hb_content").val();
			hb_content = hb_content.replace(/\n/gi, "<br>");
		location.href="teabag.bbs.update?hb_no="+$("#hb_contentUpdate2").val() + "&hb_content=" + hb_content; 
	});
}
...
function tb_dataroom_getFile() {
	$.getJSON(url, function(data) {
		...
		$.each(data.dataroom, function(i, s) {
			...
			var hd_fname = decodeURI(s.hd_fname).substring(10, 0) + "...";
			var td5 = $("<td></td>").text(hd_fname).attr("class", "tb_filesFname").attr("align", "center").attr("class", "tb_filestd2");
			var tr5 = $("<tr></tr>").append(td5);
...
function tb_member_getAllByTNo() {
	var lm_nickname = $("#hm_nickname").val();
	var tb_no1 = $("#tb_no1").val();
	...
	$.getJSON(url, function(data) {
		...
		$.each(data.teabagMember, function(i, s) {
			...
			if($("#teabagJoin").val() == 'L' ){
				if($("#teabagLeaderId").val() != s.htm_id) { 
					var deleteImg = $("<img></img>").attr("src", "resources/img/teabag/delete.png").attr("class", "tb_filesDeletePic");
					deleteATag =  $("<a></a>").attr("onclick", "goForceDeleteMember('"+s.htm_id +"', " + tb_no1 + ")").append(deleteImg); 
					var delegateDiv = $("<span></span>").text("위임").attr("class", "tb_delegateDiv");
					var delegateATag = $("<a></a>").attr("onclick", "goDelegateMember('"+s.htm_id +"', " + tb_no1 + ")").append(delegateDiv);
				} else {
					deleteATag = $("<img></img>").attr("src", "resources/img/teabag/leader.png").attr("class", "tb_filesDeletePic");
				}
			}
			var img = $("<span></span>").css("background", "url(resources/img/member/"+s.hm_photo_front+") center center no-repeat").css("background-size", "cover").attr("class", "tb_teabagProfileDiv");
			...
			var tr5 = $("<tr></tr>").append(td5);
			if (s.hm_nickname == lm_nickname) {
				img = img.attr("onclick", "goMyPage()");
				td5 = td5.attr("onclick", "goMyPage()");				
			} else {
				img = img.attr("onclick", "goPage('" + s.hm_nickname + "')");
				td5 = td5.attr("onclick", "goPage('" + s.hm_nickname + "')");
			}
...
function getTeabag(urlPlus, td) {
	...
	$.getJSON(url, function(data) {
		$.each(data.teabag, function(i, s) {
			...
			var div1 = $("<span></span>").css("background", "url(resources/img/teabag/"+s.ht_profilepic+") center center no-repeat").css("background-size", "cover").attr("class", "tb_profileDiv");
			...
			var teabagNameSpan = $("<div></div").attr("class","teabagNameSpan").html(s.ht_name);
			var teabagCategorySpan = $("<div></div>").attr("class","teabagCategorySpan").text(s.ht_category);
			var br = $("<br>")
			var span = $("<span></span>").append(teabagNameSpan,teabagCategorySpan);
	...
	$.getJSON(url, function(data) {
		if(data.pagenotice != ""){
			...
			$.each(data.pagenotice, function(i, s) {
				var hpn_content = s.hpn_content.split(" ");
				var div = $("<span></span>").text(hpn_content[0]).attr("class", "tb_noticeDiv").attr("onclick", "goTeabagPage("+s.hpn_tno+")");
				hpn_content.splice(0,1);
				var content = hpn_content.join(' ');
				var div2 = $("<span></span>").html(content+"<br>").attr("class", "tb_noticeDiv2").attr("align", "center");
				var div3 = $("<div></div>").attr("class", "tb_noticeDiv3").attr("align", "left").append(div,div2);
				
				$("#tb_personalNotice").append(div3);
			});
